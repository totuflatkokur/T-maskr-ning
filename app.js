...
function fmt(v){
  if(!v) return "";
  return new Date(v).toLocaleString("is-IS", {day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"});
}

function dateOnly(v){
  if(!v) return "-";
  return new Date(v).toLocaleDateString("is-IS");
}
...
async function hashPin(pin){ return await sha256(String(pin)); }

function setupMonthYear(){
  const now = new Date();
  $("monthSelect").innerHTML = Array.from({length:12},(_,i)=>`<option value="${i+1}" ${i===now.getMonth()?"selected":""}>${i+1}</option>`).join("");
  ...
  $("dayReportDate").value = today();
}

function bind(){
  document.querySelectorAll(".navBtn").forEach(btn => btn.onclick = () => showPage(btn.dataset.page, btn));
  ...
  $("dayReportBtn").onclick = renderDayReport;
...
async function loadEmployees(){
  ...
  if(error){ console.error(error); toast("Næ ekki að sækja starfsmenn"); employees=[]; return; }
  employees = data || [];
}

async function loadEntries(){
  const {data,error} = await db.from("time_entries").select("*").order("clock_in",{ascending:false});
  if(error){ console.error(error); toast("Næ ekki að sækja tímaskráningar"); entries=[]; return; }
  entries = data || [];
}

function activeEmployees(){ return employees.filter(e=>e.active !== false); }
function currentOpenEntry(employeeId){ return entries.find(e=>e.employee_id === employeeId && !e.clock_out); }
...
function renderClock(){
  ...
  $("employeeSelect").innerHTML = list.length
    ? list.map(e=>`<option value="${e.id}">${e.name}</option>`).join("")
    : `<option>Engir virkir starfsmenn</option>`;
}
...
async function employeeLogin(){
  const emp = activeEmployees().find(e=>e.id === $("employeeSelect").value);
...
async function clockIn(){
  ...
  if(currentOpenEntry(loggedEmployee.id)) return toast("Þú ert nú þegar skráð/ur inn");
  ...
  if(error){ console.error(error); return toast("Villa við mætingu"); }
...
async function clockOut(){
  ...
  if(error){ console.error(error); return toast("Villa við heimferð"); }
...
async function addEmployee(){
  ...
  const {error} = await db.from("employees").insert({name, pin_hash, active:true});
  if(error){ console.error(error); return toast("Villa við að bæta starfsmanni"); }
...
async function toggleEmployee(id, active){
  const {error} = await db.from("employees").update({active: !active}).eq("id", id);
  if(error){ console.error(error); return toast("Villa við breytingu"); }
  await refreshAll();
}

async function deleteEmployee(id){
  const empEntries = entries.filter(e=>e.employee_id === id);
  if(empEntries.length) return toast("Ekki hægt að eyða starfsmanni með skráningar. Óvirkjaðu hann frekar.");
  const {error} = await db.from("employees").delete().eq("id", id);
  if(error){ console.error(error); return toast("Villa við eyðingu"); }
  await refreshAll();
}

async function resetPin(id){
  const pin = prompt("Nýtt PIN:");
  if(!pin) return;
  const pin_hash = await hashPin(pin);
  const {error} = await db.from("employees").update({pin_hash, pin_code:null}).eq("id", id);
  if(error){ console.error(error); return toast("Villa við PIN"); }
  toast("PIN breytt");
  await refreshAll();
}
...
function monthlySummary(){
  ...
  return employees.map(emp=>{
    ...
    const totalHours = empEntries.reduce((s,e)=>s+hoursBetween(e.clock_in,e.clock_out),0);
...
function renderAdmin(){
  ...
  $("statEmployees").textContent = activeEmployees().length;
...
function renderDayReport(){
  const box = $("dayReportList");
  if(!box) return;
  const d = $("dayReportDate").value || today();
  const rows = entries.filter(e=>e.clock_in && e.clock_in.slice(0,10) === d);
  box.innerHTML = rows.length
    ? rows.map(e=>`
      <div class="item">
        <div><b>${e.employee_name}</b><br><span class="muted">${fmt(e.clock_in)} - ${e.clock_out ? fmt(e.clock_out) : "Í vinnu"}</span></div>
        <span class="pill ok">${hoursBetween(e.clock_in,e.clock_out).toFixed(2)} klst</span>
      </div>
    `).join("")
    : `<span class="pill">Engar skráningar þennan dag</span>`;
}

function payrollCSV(){
  const summary = monthlySummary();
  const header = ["Starfsmaður","Vaktir","Klst samtals","Í vinnu núna"];
  const rows = summary.map(r=>[r.name,r.shifts,r.totalHours.toFixed(2),r.workingNow?"Já":"Nei"].map(csvEscape).join(";"));
  downloadCSV("totu-stimpilklukka-laun.csv", header, rows);
}

function backup(){
  const data = {employees, time_entries:entries, exported_at:new Date().toISOString()};
