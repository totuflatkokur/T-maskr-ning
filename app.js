...
async function loadEmployees(){
  ...
  if(error){
    console.error(error);
    employees = [];
    return toast("Næ ekki að sækja starfsmenn");
  }
...
async function loadEntries(){
  ...
  if(error){
    console.error(error);
    entries = [];
    return toast("Næ ekki að sækja tímaskráningar");
  }
...
function renderWorkingNow(){
  ...
  $("workingNowList").innerHTML = open.length
    ? open.map(e => `
      <div class="item">
        <b>${e.employee_name || "Starfsmaður"}</b>
        <span class="pill ok">${fmt(e.clock_in)}</span>
      </div>
    `).join("")
...
async function toggleEmployee(id, active){
  ...
  if(error){
    console.error(error);
    return toast("Villa við breytingu");
  }
...
async function deleteEmployee(id){
  ...
  if(empEntries.length){
    return toast("Ekki hægt að eyða starfsmanni með skráningar. Óvirkjaðu hann frekar.");
  }
  ...
  if(error){
    console.error(error);
    return toast("Villa við eyðingu");
  }
    ...
    ? employees.map(e => `
      <div class="item ${e.active === false ? 'off' : ''}">
        <div>
          <b>${e.name}</b><br>
          <span class="muted">
            ${e.active === false ? 'Óvirkur' : 'Virkur'} · 
            ${currentOpenEntry(e.id) ? 'Í vinnu' : 'Ekki í vinnu'}
          </span>
        </div>

        <div>
          <button class="secondary mini" onclick="toggleEmployee('${e.id}', ${e.active !== false})">
            ${e.active === false ? 'Virkja' : 'Óvirkja'}
          </button>
    ...
    ? summary.map(r => `
      <div class="item ${!r.active ? 'off' : ''}">
        <div>
          <b>${r.name}</b><br>
          <span class="muted">
            ${r.shifts} vaktir · síðast: ${r.lastShift ? fmt(r.lastShift) : "-"}
          </span>
        </div>
...
function renderDayReport(){
  ...
  box.innerHTML = rows.length
    ? rows.map(e => `
      <div class="item">
        <div>
          <b>${e.employee_name}</b><br>
          <span class="muted">
            ${fmt(e.clock_in)} - ${e.clock_out ? fmt(e.clock_out) : "Í vinnu"}
          </span>
        </div>
